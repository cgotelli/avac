...
def correctingFactor1(s,theta,nu):
    ...
    """
    theta_rad = np.radians(theta) # conversion to radians
...
def export_claw_dem(xmin,xmax,ymin,ymax,nbx,nby,alt,name_file = 'topography.asc'):
    """
...
def create_cross_section(dem, x_coords, y_coords, profile_coords, num_points=1000):
    ...
    interp = RegularGridInterpolator((y_coords, x_coords), dem.data, 
...
def import_polylines(file='profil.shp'):
    ...
    if len(ligne) > 0:
        ...
        if geometry.geom_type == 'LineString':
            ...
            print(f"I import a LineString object with {len(coords)} points")
            ...
            print(f"I found a MultiLineString with {len(coords)} as the total number of points.")
            print(f"Consider providing a shapefile with a single polyline...")
            
        # Convert to numpy array for easier manipulation
        coords_array = np.array(coords)
        print("Coordinates array shape:", coords_array.shape)
    else:
